...
from dataset import PreprocessingSimple
...
def test_net_t(tester, logger, dets, img, out=None, out_cpn_input=None, flip=True):
    ...
    for i in range(test_data_mul):
        test_data.append(dets)

    # get box detections
    cls_dets = np.zeros((len(test_data), 5), dtype=np.float32)
    for i in range(len(test_data)):
        if 'bbox' not in test_data[i]:
            return
    ...
    for test_id in range(0, len(test_data), batch_size):
        ...
        for i in range(start_id, end_id):
            test_img, detail, _ = PreprocessingSimple(img, test_data[i]["bbox"], out_cpn_input)
        ...
        predict_time__ = time.time() - start_time__
        print (predict_time__)
    ...
    draw_keypoint(img, cls_skeleton[0], test_data[i]['bbox'])

    if out is not None:
        out.write(img)

    for i in range(len(cls_skeleton)):
        result = dict(image_id=test_image_id, category_id=1, score=float(round(cls_skeleton[i][-1], 4)),
...
def test_t(test_model, logger, json_path, image_path):
    import json
    ...
    def func(id):
        ...
        #return test_net_t(tester, logger, dets, image_path)

    all_res, dump_results = func(0) 

def test_for_video(test_model, logger, json_folder, video_path, out_path=None, out_cpn_input_path=None):
    dets_list = []
    ...
    def func(id):
        ...
        if out_path is not None:
            ...
            out = cv2.VideoWriter(out_path,fourcc, 20.0, (int(video_width), int(video_height)))
        
        # out_cpn_input_path = "/home/docker/Desktop/cpn_in_backextension.avi"
        out_cpn_input = None
        if out_cpn_input_path is not None:
            out_cpn_input = cv2.VideoWriter(out_cpn_input_path,fourcc, 20.0, (int(cfg.data_shape[1]), int(cfg.data_shape[0])))
        ...
        while True:
            ...
            test_net_t(tester, logger, dets_list[count], frame, out, out_cpn_input)
            count += 1   

        #img = cv2.imread(os.path.join(image_path))
        #test_net_t(tester, logger, dets, img)
        #return test_net_t(tester, logger, dets, image_path)

    # all_res, dump_results = 
    func(0) 
    return
...
if __name__ == '__main__':
    ...
    if args.test_model:
        ...
        # test_t(args.test_model, logger, "/home/docker/Desktop/test_json/test.json", "/home/docker/Desktop/test_images/0000000013.png")
        test_for_video(args.test_model, logger, "/home/docker/Desktop/push_up_out", 
            "/mnt/nas/nas4storm_disk1/xna-tsukamoto/data_for_project/AI_TRAIN-67/15プッシュアップ.MOV", 
            "/home/docker/Desktop/output.avi")
    else:
